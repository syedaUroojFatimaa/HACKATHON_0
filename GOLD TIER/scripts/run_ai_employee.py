...
loop.  Each cycle:
  ...
  3. Runs ralph_loop.py         -> autonomous step-by-step task execution
  4. Runs process_tasks.py      -> completes tasks and moves to Done/
  5. Checks approvals           -> resolves human-in-the-loop decisions
  6. Error recovery             -> quarantine stuck tasks, retry eligible ones
  7. CEO Weekly Briefing        -> generates Reports/CEO_Weekly.md once per 7 days
  8. Checks log size            -> rotates at 5 MB
...
# Load .env file into environment so all subprocesses inherit credentials
...
RALPH_LOOP_SCRIPT     = os.path.join(
    VAULT_ROOT, ".claude", "skills", "ralph-wiggum-loop", "scripts", "ralph_loop.py"
)
CEO_BRIEFING_SCRIPT   = os.path.join(
    VAULT_ROOT, ".claude", "skills", "ceo-briefing", "scripts", "ceo_briefing.py"
)
ERROR_RECOVERY_SCRIPT = os.path.join(
    VAULT_ROOT, ".claude", "skills", "error-recovery", "scripts", "error_recovery.py"
)
...
# ---------------------------------------------------------------------------
# Step 3 — Ralph Wiggum Autonomous Loop  (subprocess call)
# ---------------------------------------------------------------------------

def run_ralph_loop():
    """
    Call ralph_loop.py --run each cycle for autonomous step-by-step task
    execution.  Missing skill is silently ignored.
    """
    if not os.path.isfile(RALPH_LOOP_SCRIPT):
        return  # skill not installed — skip silently

    try:
        result = subprocess.run(
            [sys.executable, RALPH_LOOP_SCRIPT, "--run"],
            cwd=VAULT_ROOT,
            capture_output=True,
            text=True,
            timeout=120,
        ...
        if output:
            log(f"Ralph loop: {output[:300]}")
        if result.returncode != 0:
            stderr = (result.stderr or "").strip()
            if stderr:
                log(f"Ralph loop warning: {stderr[:200]}", "WARN")
    except subprocess.TimeoutExpired:
        log("ralph_loop.py timed out (120s).", "WARN")
    except OSError as err:
        log(f"Could not launch ralph_loop.py: {err}", "WARN")


# ---------------------------------------------------------------------------
# Step 4 — Process tasks  (subprocess call)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Step 5 — Check approvals (human-approval integration)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Step 6 — Error Recovery  (every cycle)
# ---------------------------------------------------------------------------

def run_error_recovery():
    """
    Call error_recovery.py --run on every cycle.
    Detects stuck tasks in Needs_Action/, quarantines them to Errors/,
    and retries eligible files after 5 minutes (up to 3 attempts).
    Missing skill is silently ignored.
    """
    if not os.path.isfile(ERROR_RECOVERY_SCRIPT):
        return  # skill not installed — skip silently

    try:
        result = subprocess.run(
            [sys.executable, ERROR_RECOVERY_SCRIPT, "--run"],
            cwd=VAULT_ROOT,
            capture_output=True,
            text=True,
            timeout=60,
        ...
        output = (result.stdout or "").strip()
        if output:
            log(f"Error recovery: {output[:300]}")
        if result.returncode != 0:
            stderr = (result.stderr or "").strip()
            if stderr:
                log(f"Error recovery warning: {stderr[:200]}", "WARN")
    except subprocess.TimeoutExpired:
        log("error_recovery.py timed out (60s).", "WARN")
    except OSError as err:
        log(f"Could not launch error_recovery.py: {err}", "WARN")


# ---------------------------------------------------------------------------
# Step 7 — CEO Weekly Briefing  (weekly, if due)
# ---------------------------------------------------------------------------

def run_ceo_briefing():
    """
    Call ceo_briefing.py --check.  Generates Reports/CEO_Weekly.md once per
    7 days; exits silently if not yet due.  Missing skill is ignored.
    """
    if not os.path.isfile(CEO_BRIEFING_SCRIPT):
        return  # skill not installed — skip silently

    try:
        result = subprocess.run(
            [sys.executable, CEO_BRIEFING_SCRIPT, "--check"],
            cwd=VAULT_ROOT,
            capture_output=True,
            text=True,
            timeout=60,
        ...
        output = (result.stdout or "").strip()
        if output:
            log(f"CEO briefing: {output[:200]}")
        if result.returncode != 0:
            stderr = (result.stderr or "").strip()
            if stderr:
                log(f"CEO briefing warning: {stderr[:200]}", "WARN")
    except subprocess.TimeoutExpired:
        log("ceo_briefing.py timed out (60s).", "WARN")
    except OSError as err:
        log(f"Could not launch ceo_briefing.py: {err}", "WARN")
...
def run_cycle(cycle_num=0):
    ...
    # Step 3: Ralph Wiggum autonomous loop (step-by-step task execution).
    run_ralph_loop()

    # Step 4: Process tasks.
    summary["processor_ok"] = run_process_tasks()

    # Step 5: Check human approvals.
    resolved = check_approvals()
    summary["approvals_resolved"] = resolved
    if resolved:
        log(f"Approvals: {resolved} resolved.")

    # Step 6: Error recovery (quarantine stuck tasks, retry eligible ones).
    run_error_recovery()

    # Step 7: CEO Weekly Briefing (generates once per 7 days).
    run_ceo_briefing()

    # Step 8: Log rotation.
    rotate_log_if_needed()  # noqa: keep numbered for readability
